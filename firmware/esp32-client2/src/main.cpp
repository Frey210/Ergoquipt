...
void setup() {
  ...
  pinMode(visualButton, INPUT_PULLUP);
  ...
  Serial.println("\nConnected to WiFi");
  Serial.println("Client 2 IP: " + WiFi.localIP().toString());

  // Setup HTTP server routes
  clientServer.on("/startTimer", HTTP_POST, handleStartTimer);
  clientServer.begin();
  
  Serial.println("HTTP server started on client 2");
}

void loop() {
  clientServer.handleClient();
  
  // Check button only if timer has started
  if (startTime > 0 && digitalRead(visualButton) == LOW) {
    handleButtonPress(currentType, currentCategory);
